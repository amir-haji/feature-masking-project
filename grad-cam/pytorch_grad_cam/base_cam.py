...
class BaseCAM:
    ...
    def cam_fc(self,
      ...
      self.model.zero_grad()
      if use_softmax:
        loss = sum([target(torch.nn.functional.softmax(output, dim=0))
                    for target, output in zip(targets, outputs)])
      else:
        loss = sum([target(output)
                    for target, output in zip(targets, outputs)])
      ...
      cam = self.get_linear_cam(input_tensor,
                              target_layer,
                              layer_activations,
                              layer_grads)
    ...
    def compute_cam_per_layer(
        ...
        for i in range(len(self.target_layers)):
            ...
            cam = self.get_cam_per_channel(input_tensor,
                                     target_layer,
                                     layer_activations,
                                     layer_grads)
            cam = np.maximum(cam, 0)
