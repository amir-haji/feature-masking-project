...
if __name__=='__main__':
    ...
    for seed in ([1,2,3]):
        ...
        trainloader, lastlayerloader, valloader, testloader = data.get_feature_loaders(f"/home/f_hosseini/data/urbancars/noaug_features_seed{seed - 1}", 128)

        sets = {'val': valloader, 'lastlayer':lastlayerloader, 'test': testloader}

        model = utils.get_fc(device, f'/home/f_hosseini/dfr-ckpts/urbancars/erm_seed{seed - 1}/ckpt.pth', num_features=2048, num_classes=2)
        ...
        for alpha in [1.0, 1.5, 2, 2.6, 3.2]:
            if not os.path.exists(f'/home/f_hosseini/data/haji/urbancars/{feat_status}/features_seed{seed}/alpha{alpha}/'):
                os.makedirs(f'/home/f_hosseini/data/haji/urbancars/{feat_status}/features_seed{seed}/alpha{alpha}/')
            ...
            for n, loader in sets.items():
                ...
                torch.save (new_features, f'/home/f_hosseini/data/haji/urbancars/{feat_status}/features_seed{seed}/alpha{alpha}/{n}_features.pt')
                torch.save(all_ys,  f'/home/f_hosseini/data/haji/urbancars/{feat_status}/features_seed{seed}/alpha{alpha}/{n}_labels.pt')
                torch.save(all_envs, f'/home/f_hosseini/data/haji/urbancars/{feat_status}/features_seed{seed}/alpha{alpha}/{n}_envs.pt')

            np.save(f'/home/f_hosseini/data/haji/urbancars/{feat_status}/features_seed{seed}/alpha{alpha}/selected_feats.npy', selected_feats)
