...
def create_mask (envs, heatmaps, alpha):
    ...
    maj_inds = [np.argwhere(val_gs == i).ravel() for i in [0, 2, 4]]
    min_inds = [np.argwhere(val_gs == i).ravel() for i in [1, 3, 5]]
    ...
    min_feats_2 = np.argwhere(val_grads[min_inds[2]].mean(0) > alpha * val_grads[maj_inds[2]].mean(0)).ravel()
    
    selected_feats = np.union1d(np.union1d(min_feats_0, min_feats_1), min_feats_2)
...
def change_dataloader_env_to_JTT(model, dataloader):
     ...
     balanced_loader = experiment.create_misc_dataloader(miscls_envs, corrcls_envs,
                                                                           ...
                                                                           dataloader=dataloader, balanced =False, dst = 'multinli')
...
if __name__=='__main__':
    ...
    for seed in ([1,2,3]):
        ...
        trainloader, lastlayerloader, valloader, testloader = data.get_feature_loaders(f"/home/f_hosseini/data/multinli/multinli_features/features_noaug_seed{seed}", 128)

        sets = {'val': valloader, 'lastlayer':lastlayerloader, 'test': testloader}

        model = utils.get_fc(device, f'/home/f_hosseini/dfr-ckpts/multinli/erm_seed{seed}/final_checkpoint.pt', num_features=768, num_classes=3)
        ...
        for alpha in [1.0, 1.5, 2, 2.6, 3.2]:
            if not os.path.exists(f'/home/f_hosseini/data/haji/multinli/{feat_status}/features_seed{seed}/alpha{alpha}/'):
                os.makedirs(f'/home/f_hosseini/data/haji/multinli/{feat_status}/features_seed{seed}/alpha{alpha}/')
            ...
            for n, loader in sets.items():
                ...
                torch.save (new_features, f'/home/f_hosseini/data/haji/multinli/{feat_status}/features_seed{seed}/alpha{alpha}/{n}_features.pt')
                torch.save(all_ys,  f'/home/f_hosseini/data/haji/multinli/{feat_status}/features_seed{seed}/alpha{alpha}/{n}_labels.pt')
                torch.save(all_envs, f'/home/f_hosseini/data/haji/multinli/{feat_status}/features_seed{seed}/alpha{alpha}/{n}_envs.pt')

            np.save(f'/home/f_hosseini/data/haji/multinli/{feat_status}/features_seed{seed}/alpha{alpha}/selected_feats.npy', selected_feats)
